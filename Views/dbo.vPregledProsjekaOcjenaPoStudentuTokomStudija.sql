...
SET ANSI_NULLS ON
GO
create VIEW [dbo].[vPregledProsjekaOcjenaPoStudentuTokomStudija]
AS
SELECT DISTINCT o.PkOsoba, o.Ime, o.Prezime, sm.Semestar,s.IdStudij, s.Smjer, s.Studij , CAST((SUM(ISNULL(sp.Ocjena,0)) / (COUNT(p.Predmet))) AS DECIMAL(2,1)) AS prosjecna_ocjena, CONVERT(INT,(CAST((SUM(ISNULL(sp.Ocjena,0)) / (COUNT(p.Predmet))) AS DECIMAL(2,1))) + 0.5) AS zakljucna_ocjena
